...
| P1-2 | `IWindowDetector::windowRectAt()` 的参数被忽略 | P1 | **已修** | src/platform/windows/WinWindowDetector.cpp | **本次复核**（2026-09-23）：正常路径已正确使用入参并做逻辑↔物理换算；`edata.found == false` 时仍回落 `GetCursorPos()`，该路径下参数被忽略。**2026-09-24 修完**：改为返回空 `QRect` —— `IWindowDetector.h` 本就把「找不到窗口」定义为空矩形，调用方 `SnapOverlay`（:133）把它变成「无悬停高亮」。**刻意没走「按 DPR 硬乘」那条路**：那只在物理原点为 (0,0) 的主屏上成立，其他屏上会落到完全不同的位置、高亮用户没指的那个窗口。⚠️ **本机不可达该路径**（Qt 屏名与 `szDevice` 总能匹配），所以**这条修复没有测试覆盖** —— 依据是读代码，不是实测 |
...
| N-9 | 其余细节（5 条） | P3 | **已修** | src/platform/windows/WinWindowDetector.cpp、src/core/IScreenCapture.h | **本次复核**逐条：① `setGeometry` 重复调用 ✅ 已删（全仓只剩 `SnapOverlay` 构造函数一处）；② `currentOverlays_` 失效 `QPointer` ✅（`closed` 时整体 `clear()`）；③ `AnnotationLayer` 两处绘制逻辑 ✅ 已去重（`paintAnnotation()` 被 `paint()` 与 `renderToImage()` 共用）；④ ✅ **2026-09-24 修完**：`EnumData edata{}` 整体值初始化（`mi` 原先只设了 `cbSize`）、`GetClassNameA`→`GetClassNameW` + `fromWCharArray`、`GetWindowLong`→`GetWindowLongPtr`（32 位形式在 64 位截断 `LONG_PTR`）；⑤ ✅ **2026-09-24 修完**：`IScreenCapture.h` 去掉 `#include <QScreen>` 改前向声明 —— `WinScreenCapture.cpp` 本就自行包含 `<QScreen>`（实测确认过包含关系无缺失） |
...
**「部分已修」只剩一条，它也就是当前唯一未完成的审查项**：

1. **P3** —— 只剩「无 CI」。`-Wall -Wextra -Wshadow` 已于 2026-09-23 打开；**测试已于 2026-09-24 落地**
   （探针并入 CMake/CTest → 19 个测试 / 627 条断言，跑一次约 11 秒）。仍没有任何 CI 配置，所以
   「每次改动都自动跑一遍」还得靠人记得跑 `ctest`。

**P1-2 与 N-9 已于 2026-09-24 修完**，状态由「部分已修」改为「已修」。其中 P1-2 那条路径在本机不可达，
**没有测试覆盖** —— 这一点写在那一行的备注里，不要在别处把它说成「已验证」。
