...
- **三轮审查清单已收尾**：41 个编号里 **40 已修、1 部分已修**。逐条状态只看 `docs/REVIEW_STATUS.md`。
  **唯一未完成的是 P3**，而它只剩「**无 CI**」—— 测试已于 2026-09-24 落地（探针并入 CMake/CTest → 19 个测试 / 627 条断言，约 12 秒），`-Wall -Wextra -Wshadow` 已于 2026-09-23 开进 CMake。
  **P1-2 与 N-9 已于 2026-09-24 修完**：P1-2 改为返回空 `QRect`（不硬乘 DPR —— 那只在物理原点为 (0,0) 的主屏上成立），但**该路径本机不可达、没有测试覆盖**（`build-review/` 里没有任何探针碰过 `windowRectAt`），依据是读代码；N-9 三项（`EnumData{}` 值初始化、`GetClassNameW`/`GetWindowLongPtr`、`IScreenCapture.h` 去 `<QScreen>` 改前向声明）全部落地。
